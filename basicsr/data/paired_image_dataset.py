...
class PairedImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        scale = self.opt.get('scale', 4)  # 默认值为 4
        ...
        if self.opt.get('phase', 'train') == 'train':
            gt_size = self.opt.get('gt_size', 128)  # 默认 128
            # random crop
            img_gt, img_lq = paired_random_crop(img_gt, img_lq, gt_size, scale, gt_path)
            # flip, rotation
            use_hflip = self.opt.get('use_hflip', True)
            use_rot = self.opt.get('use_rot', True)
            img_gt, img_lq = augment([img_gt, img_lq], use_hflip, use_rot)
